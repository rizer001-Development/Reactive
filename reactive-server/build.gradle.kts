...
paperweight {
    ...
    // Reactive start - Rebrand
    val reactive = forks.register("reactive") {
        upstream.patchDir("paperServer") {
            upstreamPath = "paper-server"
            excludes = setOf("src/minecraft", "patches", "build.gradle.kts")
            patchesDir = rootDirectory.dir("reactive-server/paper-patches")
            outputDir = rootDirectory.dir("paper-server")
        }
    }
    activeFork = reactive
    // Reactive end - Rebrand

    /*updatingMinecraft { // Reactive start - TODO - snapshot - we don't need this
        // oldPaperCommit = "d4fe85375af18bfa88f44d7c1e6a61904ae550cc"
    }*/ // Reactive end - TODO - snapshot - we don't need this
}
...
dependencies {
    implementation(project(":reactive-api")) // Reactive
    ...
    implementation("org.mozilla:rhino-runtime:1.7.14") // Reactive
    implementation("org.mozilla:rhino-engine:1.7.14") // Reactive
    implementation("dev.omega24:upnp4j:1.0") // Reactive
...
tasks.jar {
    manifest {
        ...
        val buildTime = if (build != null) Instant.now() else Instant.EPOCH // Reactive
        ...
        attributes(
            ...
            "Implementation-Title" to "Reactive", // Reactive
            "Implementation-Version" to implementationVersion,
            "Implementation-Vendor" to date,
            "Specification-Title" to "Reactive", // Reactive
            "Specification-Version" to project.version,
            "Specification-Vendor" to "rizer001", // Reactive
            "Brand-Id" to "rizer001:reactive", // Reactive
            "Brand-Name" to "Reactive", // Reactive
...
val generatedDir: java.nio.file.Path = layout.projectDirectory.dir("../paper-server/src/generated/java").asFile.toPath() // Reactive
idea {
    module {
...
/* fill { // Reactive - we don't use fill
    project("paper")
...
} */ // Reactive - we don't use fill

// tasks.register("rebuildMinecraftSourcesWithGit") {
//     group = "temp"
//
//     val patchDir = project.rootDir.resolve("reactive-server/minecraft-patches/sources").convertToPath().cleanDir()
//     val inputDir = this.project.rootDir.resolve("reactive-server/src/minecraft/java").convertToPath()
//
//     //val patchDir = project.rootDir.resolve("reactive-server/paper-patches/files").convertToPath().cleanDir()
...
// Reactive start - Add copyReactiveJar task
val reactiveServerJar = tasks.register("copyReactiveJar") {
    dependsOn("createPaperclipJar")
    doLast {
        copy {
            from(layout.buildDirectory.file("libs/reactive-paperclip-*.jar"))
            into(layout.buildDirectory.file("libs"))
            include("*.jar")
            rename { "reactive-server.jar" }
        }
        copy {
            from(layout.buildDirectory.file("libs/purpur-paperclip-*.jar"))
            into(layout.buildDirectory.file("libs"))
            include("*.jar")
            rename { "reactive-server.jar" }
        }
        println("Reactive server JAR created: reactive-server.jar")
    }
}
// Reactive end - Add copyReactiveJar task
